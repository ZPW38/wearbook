# 腕上书 · 安卓手表电子书阅读器（原生实现，不依赖 WebView）
...
- **完整 Material 3 设计**：整套界面基于 Material 3（Material You 风格），自定义 M3 色板（与网页版一致的紫调）+ M3 组件（`Surface` / `Switch` / `Slider` / `Dialog` / 进度指示器 / FAB），并专门配了一套适合小屏的字阶。
